...
@Component({
  selector: 'app-listproducture',
  templateUrl: './listproducture.component.html',
  styleUrls: ['./listproducture.component.css']
})
export class ListproductureComponent {


  onlistFilterChange($event: any) {
    throw new Error('Method not implemented.');
  }
  drawerVisible!: boolean;
  drawerTitle!: string;
  drawerData: ProcedureMaster = new ProcedureMaster();
  formTitle = 'Manage Procedures';
  dataList: any = [];
  loadingRecords = true;
  totalRecords = 1;
  pageIndex = 1;
  ...
  filterQuery: string = '';
  columns = [['NAME', 'Name'], ['IS_ACTIVE', 'Active Status'], ['Type', 'TYPE'],];
  adminId: any;
  nameVisible: boolean;
  typeVisible: boolean;
  typetext: any;
  ...
  add(): void {
    this.drawerTitle = 'Create New Procedure';
    this.drawerData = new ProcedureMaster();
    this.api.getProceduress(1, 1, 'seq_no', 'desc', '').subscribe(
      (response: HttpResponse<any>) => {
        const statusCode = response.status;
  ...
  edit(data: ProcedureMaster): void {
    this.drawerTitle = ' Update Procedure';
