...
export class PrescriptionTableComponent {
  ...
  drawerTitle!: string;
  drawerData: PrescriptionMaster = new PrescriptionMaster();
  formTitle = 'Manage Prescriptions';
  dataList: any = [];
  loadingRecords = true;
  totalRecords = 1;
  pageIndex = 1;
  ...
  filterQuery: string = '';
  columns: string[][] = [['DRUG_NAME', 'DRUG_NAME'], ['INSTRUCTION', 'INSTRUCTION'],['DATE', 'DATE'], ['IS_ACTIVE', 'Status']];
  adminId: any;
  ...
  search(reset: boolean = false) {
    ...
    if (this.citytext !== '') {
      likeQuery += (likeQuery ? ' AND ' : '') + `INSTRUCTION LIKE '%${this.citytext.trim()}%'`;
    }
    ...
    if (this.selectedCountries.length > 0) {
      ...
      likeQuery += `DRUG_ID IN (${this.selectedCountries.join(',')})`; // Update with actual field name in the DB
    }
    ...
    if (this.selectedDate) {
      ...
      likeQuery += `DATE = ${this.datePipe.transform(this.selectedDate, "yyyy-MM-dd")}`;
    }
    ...
    //   {
    //     DRUG_NAME:"Testing",
    //     INSTRUCTION:'Testing text',
    //     DATE:"12/12/2024",
       
    //   },
    //   {
    //     DRUG_NAME:"Demo",
    //     INSTRUCTION:'Demo text',
    //     DATE:"13/12/2024",
    //   }, {
    //     DRUG_NAME:"Testing",
    //     INSTRUCTION:'Testing text',
    ...
    // Call API with updated search query
     this.api.getPresciption(this.pageIndex, this.pageSize, this.sortKey, sort, likeQuery)
           .subscribe(
  ...
  add(): void {
    this.drawerTitle = 'Add New Prescription';
    this.drawerData = new PrescriptionMaster();
  ...
  edit(data: PrescriptionMaster): void {
    this.drawerTitle = ' Update Prescription';
  ...
  comparisonOptions: string[] = ['=',
    ...
    getComparisonOptions(selectedColumn: string): string[] {
      if (selectedColumn === 'DRUG_ID' || selectedColumn === 'INSTRUCTION' || selectedColumn === 'DATE') {
        return ['=', '!='];
      }
      return [
        '=',
    ...
    if (!selection1) {
      ...
      for (let i = 0; i < this.filterBox.length; i++) {
        ...
        for (let j = 0; j < this.filterBox[i]['FILTER'].length; j++) {
          ...
          console.log("this is date degfgru",selection1)
          console.log("thijrj ghtu",selection3)
      ...
      this.api.getPresciption(this.pageIndex, this.pageSize, this.sortKey, sort, newQuery)
      .subscribe(
        (response: HttpResponse<any>) => {
          const statusCode = response.status;
          // const headers = response.headers;
 ...
 applyFilter(i: number, j: number) {
  ...
  createFilterQuery1(selectedQueryIndex: number): void {
    ...
    // Call the API
    this.api.getPresciption(this.pageIndex, this.pageSize, this.sortKey, '', newQuery)
      .subscribe(
        (response: HttpResponse<any>) => {
          const statusCode = response.status;
          if (statusCode === 200) {
